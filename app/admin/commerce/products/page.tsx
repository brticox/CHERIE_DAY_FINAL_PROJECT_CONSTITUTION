    ...
    <div className="mx-auto max-w-[1680px] space-y-6 p-4 md:p-7 xl:p-9">
      <header className="flex flex-col justify-between gap-4 border-b border-cherie-lace pb-6 sm:flex-row sm:items-end">
        <div>
          ...
          <h1 className="admin-page-title mt-2">Ürünler</h1>
      ...
      {!data?.length ? (
        ...
        <>
          <section aria-label="Ürün kayıtları" className="admin-surface p-4 md:hidden">
            {data.map((product) => (
              <article key={product.id} className="admin-mobile-entity">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <Link
                      href={`/admin/commerce/products/${product.id}`}
                      className="font-semibold text-cherie-burgundy"
                    >
                      {product.name}
                    </Link>
                    <p className="mt-1 truncate text-xs text-cherie-soft-ink">
                      /{product.slug}
                    </p>
                  </div>
                  <AdminStatus value={product.status} />
                </div>
                <dl className="mt-4 grid grid-cols-2 gap-3 text-sm">
                  <div>
                    <dt className="text-xs text-cherie-soft-ink">Satış</dt>
                    <dd className="mt-1 font-medium">
                      {adminValueLabel(product.behavior_type)}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-xs text-cherie-soft-ink">Fiyat</dt>
                  ...
                  <div>
                    <dt className="text-xs text-cherie-soft-ink">Stok</dt>
                    <dd className="mt-1 font-medium">
                      {adminValueLabel(product.stock_mode)}
                    </dd>
                  </div>
                </dl>
              </article>
                ...
                <tbody className="divide-y divide-cherie-lace">
                  {data.map((product) => (
                    <tr key={product.id} className="hover:bg-cherie-paper/45">
                      ...
                      <td className="px-5 py-4">{adminValueLabel(product.stock_mode)}</td>
                      <td className="px-5 py-4">
                        <AdminStatus value={product.status} />
                      </td>
