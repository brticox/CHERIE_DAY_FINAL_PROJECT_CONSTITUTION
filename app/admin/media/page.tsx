...
export default async function Page({
  ...
  const [{ data, count, error }, products, seo, activeCount, archiveCount] =
    await Promise.all([
      request,
  ...
  return (
    <div className="mx-auto max-w-[1680px] space-y-8 p-4 md:p-7 xl:p-9">
      <AdminPageHeader
        ...
        action={
          <Link href="#media-upload" className="cherie-button-primary min-h-11">
            Yeni medya yükle
          </Link>
        }
      />
      ...
      >
        <span id="media-upload-heading" className="sr-only">
          Yeni medya yükleme alanı
        </span>
        <MediaUploader />
        <details
          id="upload-rules"
          className="mt-3 rounded-control border border-cherie-lace bg-white/40 px-4 py-3"
        >
          <summary className="cursor-pointer text-sm font-semibold text-cherie-burgundy">
            Yükleme kurallarını gör
          </summary>
          <AdminHelperText className="mt-2">
            Dosyalar sunucuda tür, gerçek dosya imzası, boyut, ölçü ve tekrar kontrolünden
            geçer. Görseller için açıklayıcı alternatif metin zorunludur.
          </AdminHelperText>
        </details>
      </section>
      ...
      {message && (
        ...
        >
          Medya işlemi tamamlanamadı. Hiçbir kayıt değiştirilmedi; dosyayı kontrol edip
      ...
      <section aria-labelledby="media-results-heading" className="space-y-5">
        <AdminSectionHeading
          ...
          description="Dosya imzası, tür, boyut ve tekrar kontrolünden geçen varlıkların güncel operasyon görünümü."
        ...
        <div className="flex flex-col justify-between gap-3 lg:flex-row lg:items-center">
          <AdminSegmentedControl
            ...
            items={[
              ...
              {
                label: 'Arşiv',
                href: '/admin/media?state=archived',
                active: archived,
          ...
          <AdminToolbar label="Medya arama araçları">
            <form className="flex w-full gap-2 sm:min-w-[420px]" role="search">
              ...
              <label className="flex min-h-11 flex-1 items-center gap-2 rounded-control border border-cherie-lace bg-white px-3 focus-within:border-cherie-brass focus-within:outline focus-within:outline-2 focus-within:outline-offset-2 focus-within:outline-cherie-focus">
                <Search
                  className="size-4 shrink-0 text-cherie-soft-ink"
                  aria-hidden="true"
                />
                ...
                <input
                  name="q"
                  defaultValue={q}
                  placeholder="Başlık veya alternatif metin ara"
        ...
        {error ? (
          ...
          hasSearch ? (
            <AdminEmptyState
              ...
              primary={{
                ...
                href: archived ? '/admin/media?state=archived' : '/admin/media',
              }}
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
            {rows.map((asset) => (
              <article
                key={asset.id}
                className="overflow-hidden rounded-card-lg border border-cherie-lace bg-white/70 shadow-sm transition-shadow hover:shadow-card"
              >
                <div className="relative flex aspect-[4/3] items-center justify-center overflow-hidden border-b border-cherie-lace bg-cherie-paper">
                  ...
                  <span className="absolute left-3 top-3">
                    <StateBadge value={asset.archived_at ? 'archived' : 'active'} />
                  </span>
                </div>
                <div className="space-y-4 p-4">
                  <div>
                    <h3 className="line-clamp-2 text-base font-semibold leading-6 text-cherie-ink">
                      {asset.title || asset.alt_text || 'Başlıksız medya'}
                    </h3>
                    <p
                      className={`mt-1 line-clamp-2 text-sm leading-5 ${asset.alt_text ? 'text-cherie-soft-ink' : 'font-semibold text-cherie-error'}`}
                    >
                  ...
                  <dl className="grid grid-cols-2 gap-x-4 gap-y-3 border-y border-cherie-lace py-3 text-sm">
                    <div>
                      <dt className="text-xs font-medium text-cherie-soft-ink">Tür</dt>
                      <dd className="mt-0.5 font-medium text-cherie-ink">
                        {asset.mime_type ?? asset.type}
                      </dd>
                    </div>
                    <div>
                      <dt className="text-xs font-medium text-cherie-soft-ink">Ölçü</dt>
                      <dd className="mt-0.5 font-medium text-cherie-ink">
                    ...
                    <div>
                      <dt className="text-xs font-medium text-cherie-soft-ink">Boyut</dt>
                      <dd className="mt-0.5 font-medium text-cherie-ink">
                        {asset.size_bytes
                    ...
                    <div>
                      <dt className="text-xs font-medium text-cherie-soft-ink">
                        Kullanım
                      </dt>
                      <dd className="mt-0.5 font-medium text-cherie-ink">
                        {asset.usage}
                  ...
                  {asset.usageLabels.length > 0 && (
                    <AdminMetadata className="line-clamp-2">
                      Kullanıldığı yer: {asset.usageLabels.join(', ')}
                    </AdminMetadata>
                  )}
                  <div className="flex items-center justify-between gap-3">
                    <AdminMetadata>
                      <AdminDate value={asset.created_at} />
                    </AdminMetadata>
                    <details className="relative">
                      <summary className="cursor-pointer rounded-control px-2 py-1 text-sm font-bold text-cherie-burgundy focus-visible:outline focus-visible:outline-2 focus-visible:outline-cherie-focus">
                        Ayarlar
                      </summary>
                      <div className="mt-3 border-t border-cherie-lace pt-3">
                        <form action={updateMediaMetadata} className="grid gap-2">
                          <input type="hidden" name="id" value={asset.id} />
                          <label className="grid gap-1 text-xs font-semibold text-cherie-ink">
                            Başlık
                            <input
                              name="title"
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            ...
                            <label className="grid gap-1 text-xs font-semibold text-cherie-ink">
                              ...
                              <input
                                ...
                                max="1"
                                step="0.01"
                                defaultValue={asset.focal_y}
                                className="cherie-field"
                              />
                            </label>
                          </div>
                          <button className="cherie-button-secondary min-h-11">
                            Kaydet
                          </button>
                        </form>
                        {!asset.archived_at && (
                          <form action={archiveMedia} className="mt-2">
                            <input type="hidden" name="id" value={asset.id} />
                            <button
                              disabled={asset.usage > 0}
                              title={
                                asset.usage > 0 ? 'Kullanımdaki medya arşivlenemez.' : ''
                              }
                              className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-control text-sm font-bold text-cherie-error hover:bg-cherie-error/10 disabled:cursor-not-allowed disabled:opacity-40"
                            >
                              <Archive className="size-4" aria-hidden="true" /> Güvenli
                              arşivle
                            </button>
                          </form>
                        )}
        ...
        {!error && rows.length > 0 && (
          <AdminMetadata>
            Toplam {count ?? 0} kayıt · Bu görünüm en güncel 100 kaydı gösterir.
          </AdminMetadata>
        )}
      </section>
    </div>
