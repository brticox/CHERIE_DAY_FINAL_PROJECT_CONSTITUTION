...
export function SiteHeader() {
  ...
  return (
    <>
      <header
        className={cn(
          ...
          overSky
            ...
            : 'bg-cherie-ivory/88 border-b border-cherie-brass/35 shadow-[0_1px_0_rgba(176,138,87,0.18)] backdrop-blur-[14px]',
        )}
        data-chrome={overSky ? 'sky' : 'paper'}
      >
        <div
          className={cn(
            'cherie-container flex h-[4.5rem] items-center justify-between gap-4 transition-colors duration-300',
            overSky ? 'text-cherie-ivory' : 'text-cherie-ink',
          )}
        >
          <div className="flex items-center gap-3">
            <MobileNav overSky={overSky} />
          ...
          <div
            className={cn('flex items-center gap-1', overSky && '[&_svg]:stroke-[1.5]')}
          >
            <Button
              asChild
              variant="ghost"
      ...
      {pathname !== '/' ? <div aria-hidden="true" className="h-[4.5rem]" /> : null}
    </>
  );
}
